...
The complete pinned dependency tree lives in `requirements.lock`. It is the CM-3 artifact for this repo: the exact versions used to produce any given OSCAL SAR, recorded once and version-controlled. `requirements.txt` documents direct-dependency intent. The looser compatible-release pins in `pyproject.toml` define the contract for downstream installers. The three files separate "what the package needs" from "what we shipped against."
...
> No real credentials appear in the sample. The `secret-scanner` schema records only the detection regex (`pattern_matched`), a file path, and a severity. Never the matched secret. So the evidence artifact cannot leak a credential by structure.
