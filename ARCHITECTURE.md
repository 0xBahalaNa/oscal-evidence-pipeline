...
The pipeline solves a single problem: **the JSON shapes produced by individual audit tools are not the JSON shape an OSCAL-consuming system expects.** Without a transformation layer, every audit tool would need to be modified to emit OSCAL Assessment Results directly. That couples the tools to the OSCAL schema version and makes each tool harder to maintain.
...
**Detection strategy:** Each upstream tool's JSON has a stable top-level structure. For example, `secret-scanner --output json` produces `{"scan_metadata": {...}, "findings": [...], "summary": {...}}`. Schema fingerprinting matches on the top-level key set rather than requiring upstream tools to add a `source_tool` field. That keeps the upstream tools loose-coupled.
...
- **Adapter `matches()` raises:** raise `AdapterMatchError` wrapping the original exception with the offending adapter's registry key and class name. A broken adapter must not silently mask a correct one downstream. That's the same silent-ambiguity failure mode the multi-match policy guards against, and it **halts the run** too (same scope as the multi-match case).
...
3. **It's where the portfolio compounds the most.** Every other audit tool the portfolio ships becomes more valuable when its output is OSCAL-consumable. SAR is the format that makes that true.
...
The pipeline doesn't compute KSIs itself. That's downstream. It produces the input format the KSI collectors need.
...
Validation happens in three layers. Each is a **superset** of the one before it: a SAR that passes Layer 3 has also passed Layers 1 and 2. Each catches a class of error the prior layer *relaxes*.
